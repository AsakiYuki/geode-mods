...
namespace AsakiYuki {
    class Color {
        protected:
        static inline float st = 0.f;
        public:
        static void update(float dt) {
            st += dt;
        }
        static ccColor3B hexToColor(int hex) {
            return ccc3(static_cast<GLubyte>((hex >> 16) & 0xff), static_cast<GLubyte>((hex >> 8) & 0xff), static_cast<GLubyte>((hex >> 0) & 0xff));
        }
        static ccColor3B getPastelColor(float e = 1.f, bool reverse = false) {
            std::vector<int> pastel = {
            ...
            float delta = fmod(st, e) / e;
            if (reverse) return hexToColor(pastel[((int)(delta * pastel.size() + pastel.size() / 2)) % pastel.size()]);
            else return hexToColor(pastel[(int)(delta * pastel.size())]);
        }

        static ccColor3B getFadeColor(ccColor3B c1, ccColor3B c2, float e = 1.f) {
            float delta = fmod(st, e);
            delta = ((delta <= (e / 2.f)) ? delta : (e - delta)) / (e / 2.f);
            int r = c2.r - c1.r;
            int g = c2.g - c1.g;
            int b = c2.b - c1.b;

            int offsetR = (r == 0) ? 0 : (r * delta);
            int offsetG = (g == 0) ? 0 : (g * delta);
            int offsetB = (b == 0) ? 0 : (b * delta);
        ...
        static ccColor3B hsvToRGB(const ccHSVValue hsv) {
            float f = hsv.h / 60.f - floor(hsv.h / 60.f);
            float p = hsv.v * (1 - hsv.s);
            float q = hsv.v * (1 - f * hsv.s);
            float t = hsv.v * (1 - (1 - f) * hsv.s);
            float r, g, b;
            switch(static_cast<int>(fmod(hsv.h / 60.f, 6.f))) {
        ...
        static ccColor3B getChromaColor(float e = 1.f, bool reverse = false) {
            if (reverse) {
                ccHSVValue hsv = cchsv((st * 180.f) / e, 1.f, 1.f, true, true);
                hsv.h = fmod(hsv.h + 180.f, 360.f);
                return hsvToRGB(hsv);
            }
            else return hsvToRGB(cchsv((st * 180.f) / e, 1.f, 1.f, true, true));
        };

        static ccColor3B getColor(
            ModTypes::ColorType type,
            float time = 1.f,
            bool reverse = false,
            ccColor3B color1 = GameManager::sharedState()->colorForIdx(GameManager::sharedState()->getPlayerColor()),
            ccColor3B color2 = GameManager::sharedState()->colorForIdx(GameManager::sharedState()->getPlayerColor2())
        ) {
            switch (type) {
                case ModTypes::ColorType::FADE: {
                    return getFadeColor(color1, color2, time);
                } break;
                case ModTypes::ColorType::PASTEL: {
                    return getPastelColor(time, reverse);
                } break;
                case ModTypes::ColorType::RAINBOW: {
                    return getChromaColor(time, reverse);
                } break;
        ...
        static ccColor3B playerColor(bool isSecondPlayer = false) {
            auto mod = Mod::get();
            ...
            int colorID = 0;
            if (ingameLayer != nullptr && (dualType == 0 || !dualMode)) colorID = AsakiYuki::Save::getPlayerFrame(isSecondPlayer ? ModTypes::Gamemode::COLOR_2 : ModTypes::Gamemode::COLOR_1);
            else colorID = AsakiYuki::Save::getPlayerFrame(ModTypes::Gamemode::COLOR_1, isSecondPlayer);

            if (colorID == 1001) return Color::getColor(ModTypes::ColorType::PASTEL,
            mod->getSavedValue<float>("asakiyuki:color_cycle_speed", 1.f));
            else if (colorID == 1002) return Color::getColor(ModTypes::ColorType::PASTEL,
            mod->getSavedValue<float>("asakiyuki:color_cycle_speed", 1.f), true);
            else if (colorID == 1003) return Color::getColor(ModTypes::ColorType::RAINBOW,
            mod->getSavedValue<float>("asakiyuki:color_cycle_speed", 1.f));
            else if (colorID == 1004) return Color::getColor(ModTypes::ColorType::RAINBOW,
            mod->getSavedValue<float>("asakiyuki:color_cycle_speed", 1.f), true);

            else return GameManager::sharedState()->colorForIdx(colorID);
        }
        static ccColor3B playerColor2(bool isSecondPlayer = false) {
            auto mod = Mod::get();
            GJBaseGameLayer* ingameLayer = (PlayLayer::get() != nullptr) ? PlayLayer::get() : static_cast<GJBaseGameLayer*>(LevelEditorLayer::get());
            int dualType = mod->getSavedValue<int>("asakiyuki:dual_types", 0);
            bool dualMode = mod->getSavedValue<bool>("asakiyuki:custom_dual_mod", false);
            ...
            int colorID = 0;
            if (ingameLayer != nullptr && (dualType == 0 || !dualMode)) colorID = AsakiYuki::Save::getPlayerFrame(isSecondPlayer ? ModTypes::Gamemode::COLOR_1 : ModTypes::Gamemode::COLOR_2);
            else colorID = AsakiYuki::Save::getPlayerFrame(ModTypes::Gamemode::COLOR_2, isSecondPlayer);
        ...
        static ccColor3B playerGlowColor(bool isSecondPlayer = false) {
            auto mod = Mod::get();
            GJBaseGameLayer* ingameLayer = (PlayLayer::get() != nullptr) ? PlayLayer::get() : static_cast<GJBaseGameLayer*>(LevelEditorLayer::get());
            int dualType = mod->getSavedValue<int>("asakiyuki:dual_types", 0);
            bool dualMode = mod->getSavedValue<bool>("asakiyuki:custom_dual_mod", false);
            ...
            int colorID = 0;
            if (ingameLayer != nullptr && (dualType == 0 && dualMode)) colorID = AsakiYuki::Save::getPlayerFrame(isSecondPlayer ? ModTypes::Gamemode::COLOR_1 : ModTypes::Gamemode::COLOR_2);
            else colorID = AsakiYuki::Save::getPlayerFrame(ModTypes::Gamemode::COLOR_GLOW, isSecondPlayer);
        ...
        static ccColor3B playerWaveTrailColor(bool isSecondPlayer = false) {
            auto colorID = AsakiYuki::Save::getCustomPlayer(ModTypes::Custom::WAVE_TRAIL, isSecondPlayer);
            auto mod = Mod::get();
            if (colorID == 1001) return Color::getColor(ModTypes::ColorType::PASTEL,
            mod->getSavedValue<float>("asakiyuki:color_cycle_speed", 1.f));
            else if (colorID == 1002) return Color::getColor(ModTypes::ColorType::PASTEL,
